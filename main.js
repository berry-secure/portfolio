const translations = {
  pl: {
    ...
    project_signal:
      "CMS, player Android TV, harmonogram emisji, proof-of-play, heartbeat urządzeń, komendy live, role użytkowników i model multi-tenant.",
    project_berry:
    ...
    exp_1:
      "Projekty IT/AV, wyceny, rozwiązania sieciowe, wsparcie działu obsługi klienta i komunikacja z klientami w Europie.",
    exp_2:
      "Diagnostyka zgłoszeń, MS365 Admin, Azure, kontrolery domeny, Kaseya, TeamViewer i raportowanie incydentów.",
    exp_3_title: "Redakcja, sprzedaż, przedsiębiorczość",
    exp_3:
  ...
  en: {
    ...
    project_signal:
      "CMS, Android TV player, playback scheduling, proof-of-play, device heartbeats, live commands, user roles and a multi-tenant model.",
    project_berry:
    ...
    exp_1:
      "IT/AV projects, quotes, network solutions, customer support enablement and communication with clients across Europe.",
    exp_2:
      "Issue diagnosis, MS365 Admin, Azure, domain controllers, Kaseya, TeamViewer and incident reporting.",
    exp_3_title: "Editorial work, sales, entrepreneurship",
    exp_3:
...
function setLanguage(lang) {
  ...
  document.querySelectorAll("[data-cv-link], [data-cv-link-bottom]").forEach((link) => {
    link.setAttribute("href", cvLinks[lang]);
  });
...
setLanguage(localStorage.getItem("portfolio-lang") || "pl");
...
const nodes = [
  { x: 0.12, y: 0.28, r: 4, c: palette.teal },
  { x: 0.28, y: 0.44, r: 3, c: palette.gold },
  { x: 0.48, y: 0.22, r: 5, c: palette.coral },
  { x: 0.67, y: 0.5, r: 3, c: palette.violet },
  { x: 0.84, y: 0.31, r: 4, c: palette.teal },
  { x: 0.74, y: 0.72, r: 4, c: palette.gold },
  { x: 0.36, y: 0.78, r: 3, c: palette.coral },
  { x: 0.16, y: 0.66, r: 4, c: palette.violet },
];
...
function drawFrame(time) {
  ...
  ctx.strokeStyle = "rgba(255,253,248,0.075)";
  ctx.lineWidth = 1;
  const grid = 56;
  ...
  const points = nodes.map((node, index) => ({
    x: node.x * width + Math.sin(t * 5 + index) * 18,
    y: node.y * height + Math.cos(t * 4 + index * 0.8) * 14,
    r: node.r,
    c: node.c,
  }));
  ...
  points.forEach((point, index) => {
    ...
    const nextTwo = points[(index + 3) % points.length];
    drawLink(point, next, index);
    drawLink(point, nextTwo, index + 3, 0.42);
  });

  points.forEach((point, index) => {
    ctx.beginPath();
    ctx.arc(point.x, point.y, point.r + 8, 0, Math.PI * 2);
    ctx.fillStyle = point.c + "24";
    ctx.fill();
    ctx.beginPath();
  ...
  drawPackets(points, time);

  if (!reducedMotion) requestAnimationFrame(drawFrame);
}

function drawLink(a, b, index, alpha = 0.72) {
  ctx.beginPath();
  ctx.moveTo(a.x, a.y);
  const cx = (a.x + b.x) / 2 + Math.sin(index) * 34;
  const cy = (a.y + b.y) / 2 + Math.cos(index) * 30;
  ctx.quadraticCurveTo(cx, cy, b.x, b.y);
  ctx.strokeStyle = `rgba(255,253,248,${alpha * 0.28})`;
  ctx.stroke();
}

function drawPackets(points, time) {
  for (let index = 0; index < points.length; index += 1) {
    const a = points[index];
    const b = points[(index + 1) % points.length];
    const phase = (time * 0.00016 + index / points.length) % 1;
    const x = a.x + (b.x - a.x) * phase;
    const y = a.y + (b.y - a.y) * phase;
    ctx.fillStyle = index % 3 === 0 ? palette.gold : index % 3 === 1 ? palette.teal : palette.coral;
